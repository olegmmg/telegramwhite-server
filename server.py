...
class DB:
        ...
        try:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    status TEXT DEFAULT 'offline', created_at BIGINT NOT NULL, last_seen BIGINT NOT NULL
                );
                CREATE TABLE IF NOT EXISTS chats (
                    id SERIAL PRIMARY KEY, name TEXT NOT NULL,
                    type TEXT NOT NULL DEFAULT 'group',
                    created_by INTEGER REFERENCES users(id), created_at BIGINT NOT NULL
                );
                CREATE TABLE IF NOT EXISTS chat_members (
                    chat_id INTEGER REFERENCES chats(id) ON DELETE CASCADE,
                    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
                    joined_at BIGINT NOT NULL, PRIMARY KEY (chat_id, user_id)
                );
                CREATE TABLE IF NOT EXISTS messages (
                    id SERIAL PRIMARY KEY,
                    chat_id INTEGER REFERENCES chats(id) ON DELETE CASCADE,
                    user_id INTEGER REFERENCES users(id), username TEXT NOT NULL,
                    text TEXT NOT NULL, created_at BIGINT NOT NULL, deleted BOOLEAN DEFAULT FALSE
                );
    ...
    def verify_user(self, username, password):
        return self._one("SELECT id,username,email FROM users WHERE username=%s AND password=%s", (username, self.hash(password)))
    ...
    def new_session(self, uid):
        ...
        now = int(time.time())
        self._run("INSERT INTO sessions (token,user_id,created_at,expires_at) VALUES (%s,%s,%s,%s)", (token, uid, now, now+86400*30))
        return token
    ...
    def user_chats(self, uid):
        return self._all("""
            ...
            FROM chats c JOIN chat_members cm ON cm.chat_id=c.id WHERE cm.user_id=%s
    ...
    def get_or_create_private(self, uid1, uid2, name2):
        ...
        try:
            ...
            if ex:
                return dict(ex)
            cur.execute("INSERT INTO chats (name,type,created_by,created_at) VALUES (%s,'private',%s,%s) RETURNING id,name", (name2, uid1, now))
            chat = dict(cur.fetchone())
            cur.execute("INSERT INTO chat_members (chat_id,user_id,joined_at) VALUES (%s,%s,%s),(%s,%s,%s)", (chat['id'],uid1,now,chat['id'],uid2,now))
            c.commit()
            return chat
        except Exception as e:
            c.rollback()
    ...
    def get_msgs(self, chat_id, limit=50, before_id=None):
        if before_id:
            rows = self._all("SELECT id,user_id,username,text,created_at FROM messages WHERE chat_id=%s AND deleted=FALSE AND id<%s ORDER BY created_at DESC LIMIT %s", (chat_id, before_id, limit))
        else:
            rows = self._all("SELECT id,user_id,username,text,created_at FROM messages WHERE chat_id=%s AND deleted=FALSE ORDER BY created_at DESC LIMIT %s", (chat_id, limit))
...
async def broadcast(data, chat_id=None, skip=None):
    ...
    targets = [info["ws"] for uid, info in conns.items() if info["ws"] is not skip and (chat_id is None or info.get("current_chat") == chat_id)]
    if targets:
        ...
        async for raw in ws:
            ...
            if t == "send_message":
                ...
                if not text or len(text) > 4096:
                    await send(ws, {"type":"error","message":"Пустое сообщение"}); continue
                ...
                tname = d.get("username","").strip()
                target = db._one("SELECT id FROM users WHERE username=%s", (tname,))
                if not target:
                    await send(ws, {"type":"error","message":"Пользователь не найден"}); continue
                chat = db.get_or_create_private(uid, target['id'], tname)
                await send(ws, {"type":"private_chat_created","chat":{"id":chat["id"],"name":tname,"type":"private"},"messages":db.get_msgs(chat['id'])})
                if target['id'] in conns:
                    await send(conns[target['id']]["ws"], {"type":"new_private_chat","chat":{"id":chat["id"],"name":conns[uid]["username"],"type":"private"}})
            elif t == "logout":
